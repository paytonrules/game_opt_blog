...
<pre>
  C = A + B; // ../PA4_Implicit/stressTest.cpp:44: error: no matching function for call to 'Vect::Vect(Vect&)'
...
{% highlight C %}
{
	...
	{
		return ( Vect(this->x + tmp.x, this->y + tmp.y, this->z + tmp.z ) ); // ../PA4_Implicit/PA4_A.h: In member function 'Vect Vect::operator+(Vect) const':
